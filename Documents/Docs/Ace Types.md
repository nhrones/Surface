...
/**
 * This class provides an easy way to treat the document 
 * as a stream of tokens, and provides methods to iterate 
 * over these tokens.
 */
 ...
 * EditSession can be attached to only one Document. Same Document can be attached to
 * several EditSessions.
 */
 ...
 * The `Editor` manages the EditSession (which manages Documents), 
 * as well as the VirtualRenderer, which draws everything to the screen.
 * 
 * Event sessions dealing with the mouse and keyboard are bubbled up 
 * from `Document` to the `Editor`, which decides what to do with them.
 */
